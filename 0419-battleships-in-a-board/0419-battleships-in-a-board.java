class Solution {
    ...
    public static void check(char grid[][], boolean check[][], int c, int r) {
        ...
        if (grid[r][c] == 'X' && check[r][c] == false) {
            check[r][c] = true;
        }
        if (r - 1 >= 0 && grid[r - 1][c] =='X' && check[r - 1][c] == false) {
            check[r - 1][c] = true;
            check(grid, check, c, r - 1);
        }
        if (c - 1 >= 0 && grid[r][c - 1] == 'X' && check[r][c - 1] == false) {
            check[r][c - 1] = true;
            check(grid, check, c - 1, r);

        }
        if (r + 1 < grid.length && grid[r + 1][c] =='X' && check[r + 1][c] == false) {
            check[r + 1][c] = true;
            check(grid, check, c, r + 1);

        }
        if (c + 1 < grid[0].length && grid[r][c + 1] == 'X' && check[r][c + 1] == false) {
            check[r][c + 1] = true;
            check(grid, check, c + 1, r);
        }
    }
